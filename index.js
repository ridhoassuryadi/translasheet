...
module.exports = (args) => {
    ...
    const CONFIG = require(path.join(PROJECT_DIRECTORY, args.config || DEFAULT_CONFIG_FILE_NAME));
    ...
    const OUTPUT = path.join(PROJECT_DIRECTORY, setOutput(args.output || DEFAULT_OUTPUT_DIR))
    ...
    function writeToJson(translations) {
        ...
        // Output type single
        if (OUTPUT_TYPE == "single") {
            fs.writeFileSync(OUTPUT, getTranslationBasedOnCountry(translations, country))

            // Output type multiple
        } else {
            Object.entries(translations).forEach(
                ([key, value]) => {
                    fs.writeFileSync(OUTPUT + translations[key]["key"] + ".json", JSON.stringify(translations[key]["data"], null, 2))
                })
        }
    }
        ...
        response
            ...
            .on('end', () => {
                ...
                function assignTranslationKey(translations, rows) {
                    getHeaderKey(rows).map(
                        (curr, i) => assignCountryKey(i, LIST_COUNTRY[curr], translations)
                    )
                    return {
                        translations,
                        rows
                ...
                spinner.stop()
                writeToJson(assignDataBasedOnCountry(assignTranslationKey({}, splitBodyData(body))))
